...
//날         짜  : 2009-02-06
//======================================================================
package console.user_window.dao;
...
//=======================================================================================
//					전체 레코드수 구하기UserWindowList
//=======================================================================================
	public ResultSetTray UserWindowList(Connection conn,Tray reqTray){
		...
		userid 				=	reqTray.getString("userid"); 		 // 사용자 아이디 
		
		
		userid				=	"ngstest";							 // 임시 아이디 값 
		
		
		cal1 				= 	Util.ReplaceText("/", "" , reqTray.get("cal1"));
		cal2 				= 	Util.ReplaceText("/", "" , reqTray.get("cal2"));
		
		query=new StringBuffer();
		
		query.append("	select e.user_id , e.title , e.from_dt , 				\n"); 
		query.append("		e.to_dt , reg_dt , e.admit_flag,					\n");
		query.append("			e.job_no , e.status , e.cert_id 				\n");
		query.append("	from TBL_JOB_LIST e, 									\n");
		query.append("		( select distinct job_no 							\n");
		query.append("				from TBL_JOB_SYSTEM c, 						\n");
		query.append("		( select b.system_id, b.system_name 				\n");
		query.append("				from TBL_SERVICE a, TBL_SYSTEM b			\n");
...
//		IF ($find1!=""){
//			$SQL = $SQL. " and d.".$key1." = '".check_web2db($find1)."' ";
//		}
		query.append("  ) f 													\n");
		query.append(" 			where e.job_no = f.job_no and e.admit_flag >=0  \n");
		query.append(" 					and user_id = '"+userid+"' 				\n");
		if (!cal1.equals("")  &&  !cal2.equals("")){
		query.append(" 		and date_format(e.from_dt,'%Y%m%d') >= '"+cal1+"' 	\n");
		query.append("		and date_format(e.from_dt,'%Y%m%d') <= '"+cal2+"'	\n");
		}
